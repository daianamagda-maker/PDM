...
export default function PDMDealershipApp() {
  ...
  const selectVehicle = (vehicle: Vehicle) => {
    setVehicleSearch(vehicle.model);
    setSaleForm({
      ...saleForm,
      model: vehicle.model,
  ...
  const vehicleSuggestions = vehicles
    .filter((v) => v.model.toLowerCase().includes(vehicleSearch.toLowerCase()))
    .slice(0, 10);
          ...
          {active === "Ventes" && (
            <section className="grid two">
              <Card>
                ...
                <input
                  placeholder="Rechercher un modèle véhicule"
                  value={vehicleSearch}
                  onChange={(e) => {
                    setVehicleSearch(e.target.value);
                    setSaleForm({ ...saleForm, model: e.target.value, category: "", price: "" });
                  }}
                />
                {vehicleSearch && vehicleSuggestions.length > 0 && (
                  <div className="suggestions">
                    {vehicleSuggestions.map((v) => (
                      <button key={v.model} type="button" onClick={() => selectVehicle(v)}>
