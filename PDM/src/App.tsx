...
const API_URL = "COLLE_ICI_TON_URL_APPS_SCRIPT_PDM";
...
export default function PDMDealershipApp() {
  ...
  const [saleForm, setSaleForm] = useState({ plate: "", model: "", price: "", owner: "" });
  ...
  const loadActivePage = async (page: string) => {
    ...
    if (page === "Ventes") await fetchSales();
  ...
  const addSale = async () => {
    ...
    setSaleForm({ plate: "", model: "", price: "", owner: "" });
    fetchSales();
  };
          ...
          {active === "Ventes" && (
            <section className="grid two">
              <Card>
                ...
                <input placeholder="Modèle véhicule" value={saleForm.model} onChange={(e) => setSaleForm({ ...saleForm, model: e.target.value })} />
              ...
              <Card>
                ...
                <Table headers={["Date", "Employé", "Plaque", "Modèle", "Prix", "Propriétaire", "Prime"]}>
                  {filteredSales.map((s) => <tr key={s.id}><td>{s.date}</td><td><b>{s.employee}</b></td><td>{s.plate}</td><td>{s.model}</td><td>{s.price.toLocaleString("fr-FR")} $</td><td>{s.owner}</td><td className="text-green">{s.bonus.toLocaleString("fr-FR")} $</td></tr>)}
                </Table>
              </Card>
            </section>
          )}
  ...
  return (
    <style>{`
      * { box-sizing: border-box; }
      body { margin: 0; font-family: Inter, Arial, sans-serif; background: #05070d; }
      .app { min-height: 100vh; background: radial-gradient(circle at top left, rgba(37,99,235,.24), transparent 32%), radial-gradient(circle at top right, rgba(225,29,72,.22), transparent 30%), #05070d; color: #f8fafc; }
      ...
      .topbar h2 { margin: 0; font-size: 34px; font-style: italic; font-weight: 1000; }
