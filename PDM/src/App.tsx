...
export default function PDMDealershipApp() {
  ...
  const [saleForm, setSaleForm] = useState({ plate: "", model: "", price: "", owner: "", hasLicense: false });
  ...
  const addSale = async () => {
    ...
    setSaleForm({ plate: "", model: "", price: "", owner: "", hasLicense: false });
    fetchSales();
  };
          ...
          {active === "Ventes" && (
            <section className="grid two">
              <Card>
                ...
                <input list="vehicles-list" placeholder="Modèle véhicule" value={saleForm.model} onChange={(e) => handleVehicleChange(e.target.value)} />
                <datalist id="vehicles-list">
                  {vehicles.map((v) => <option key={v.model} value={v.model}>{v.category}</option>)}
                </datalist>
              ...
              <Card>
                ...
                <Table headers={["Date", "Employé", "Plaque", "Modèle", "Prix", "Propriétaire", "Permis", "Prime"]}>
                  {filteredSales.map((s) => <tr key={s.id}><td>{s.date}</td><td><b>{s.employee}</b></td><td>{s.plate}</td><td>{s.model}</td><td>{s.price.toLocaleString("fr-FR")} $</td><td>{s.owner}</td><td className="license-cell">{s.hasLicense ? "✅" : "❌"}</td><td className="text-green">{s.bonus.toLocaleString("fr-FR")} $</td></tr>)}
                </Table>
              </Card>
            </section>
          )}
